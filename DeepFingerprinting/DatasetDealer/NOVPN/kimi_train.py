...
# ==================== 2. 配置文件 ====================
CONFIG = {
    'data_dir': '/home/pcz/DL/ML_DL/DeepFingerprinting/DatasetDealer/NOVPN/novpn_npz_longflows_all',
    'label_mapping': '/home/pcz/DL/ML_DL/DeepFingerprinting/DatasetDealer/NOVPN/novpn_npz_longflows_all/labels.json',
...
class TrafficDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 确保flow是数值类型，处理object类型数组
        if flow.dtype == np.object_:
            # 如果是object类型，尝试转换为int8
            flow = np.array(flow, dtype=np.int8)
...
def get_data_loaders(config):
    ...
    for file_path in tqdm(npz_files, desc="Loading all samples"):
        try:
            ...
            for i in range(len(flows)):
                ...
                sample = (flow, label_id)
                all_samples.append(sample)

                # 按类别分组
                if label_id not in samples_by_class:
                    samples_by_class[label_id] = []
                samples_by_class[label_id].append(sample)
    ...
    if skipped > 0:
        print(f"Warning: 跳过 {skipped} 个未知标签的样本")
    ...
    print('总类别数量:', len(samples_by_class.items()))
    sample_num = 0
    for class_id, samples in samples_by_class.items():
        n_total = len(samples)
        if n_total < 10:  # 如果某个类别样本太少
            # train_samples.extend(samples)
            print(f"Warning: 类别 {class_id} 只有 {n_total} 个样本，舍弃")
            continue
