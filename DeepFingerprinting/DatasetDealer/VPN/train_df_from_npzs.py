...
def setup_device_amp() -> Tuple[torch.device, torch.dtype, bool]:
    ...
    if device.type == "cuda":
        ...
        # Prefer new TF32 / matmul settings if available, fallback safely
...
def main() -> None:
    ...
    te_loss, te_acc, te_mf1 = evaluate(model, test_loader, device, amp_dtype, num_classes, epoch_idx=0, tag="TEST")
