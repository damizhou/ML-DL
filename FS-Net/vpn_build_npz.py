...
# -*- coding: utf-8 -*-
"""
vpn_build_npz.py - FS-Net VPN 数据处理

功能：
- 递归扫描 ROOT 目录（按子目录名作为 label）下的 .pcap / .pcapng
- 多进程并行处理 PCAP 文件
- 提取数据包长度序列（带方向：正值=出站，负值=入站）
- 每个 label 的所有流合并为一个 NPZ 文件
- 输出格式兼容 FS-Net 训练脚本

输出 NPZ 结构：
    - sequences: object 数组，元素为变长 int16 数组（±长度值）
    - labels: int64 数组，类别标签
    - label_map: 保存在同目录的 labels.json
"""
...
OUT_ROOT = Path(__file__).resolve().parent / "vpn_fsnet_data"  # 输出目录
MAX_PROCS = 16  # 并发进程数
MIN_PKTS_PER_FLOW = 10  # 最小数据包数
MAX_SEQ_LEN = 100  # 最大序列长度（截断）
MAX_PKT_LEN = 1500  # 最大数据包长度（MTU）
...
def extract_flows_from_pcap(pcap_path: str) -> List[np.ndarray]:
    ...
    返回: List[np.ndarray]，每个元素是一个流的长度序列（int16，带方向）
    """
    ...
    # 按五元组聚合流: key -> {"origin": (src, sport, dst, dport), "lengths": []}
    ...
    for ts, buf in _iter_packets(pcap_path):
        ...
        # 限制数据包长度
        pkt_len = min(pkt_len, MAX_PKT_LEN)

        # 初始化流
        if key not in flows:
            flows[key] = {"origin": (src_ip.compressed, sport, dst_ip.compressed, dport), "lengths": []}

        # 添加带方向的长度
        signed_len = direction * pkt_len
        flows[key]["lengths"].append(signed_len)

    # 过滤并转换
    result = []
    for flow_data in flows.values():
        lengths = flow_data["lengths"]
        if len(lengths) >= MIN_PKTS_PER_FLOW:
            # 截断到最大长度
            lengths = lengths[:MAX_SEQ_LEN]
...
def main():
    print(f"[FS-Net VPN Data Processor]")
    print(f"输入目录: {ROOT}")
    ...
    for label in labels:
        ...
        np.savez_compressed(
            out_path,
            sequences=np.array(flows, dtype=object),
            label=label,
            label_id=label_id
        )
