...
# Output path
OUTPUT_DIR = Path(__file__).resolve().parent / "data" / "cic_iot_2022"
...
def process_iscxvpn_dataset():
    ...
    for label_id in valid_classes:
        ...
        # Sanitize label name for filesystem (replace / with _)
        safe_label_name = label_name.replace('/', '_')
        mask = labels == label_id
        if mask.sum() > 0:
            class_images = images[mask]
            class_path = output_path / f'{safe_label_name}.npz'
            np.savez_compressed(
                class_path,
                images=class_images,
