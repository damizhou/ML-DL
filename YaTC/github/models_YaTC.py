...
class TrafficTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_features(self, x, attn_mask=None):
        """
        兼容新版 timm：接受 attn_mask（如果上游传入），并在可能时把它传给 block。
        这里保持行为与原实现一致（当 block 不接受 attn_mask 时直接调用）。
        """
        x = self.patch_embed(x)

        # 如果 pos_embed 是可广播的，保持原逻辑
        if hasattr(self, "pos_embed") and self.pos_embed is not None:
            # 如果 pos_embed 形状 (1, N, C) 或类似，按原代码相加
            x = x + self.pos_embed

        if hasattr(self, "pos_drop"):
            x = self.pos_drop(x)

        # blocks 里某些 block 可能支持 attn_mask 参数（新版 timm）
        for blk in self.blocks:
            try:
                # 尝试传入 attn_mask（若 block 支持）
                x = blk(x, attn_mask=attn_mask)
            except TypeError:
                # block 不接受 attn_mask，退回到原调用
                x = blk(x)
...
class MaskedAutoencoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        # 下面是原来的 forward 剩余逻辑（比如分类头）
        x = self.head(x) if hasattr(self, "head") else x
        return x
